...
5. Identify any unique or innovative features that are very specific to the language and explain how they affect the way programmers write and structure code.
   ...
   6. Another interesting note about Lua is that it does not use POSIX regex for pattern matching. This is because POSIX regex implementation takes over 4,000 lines of code which is bigger than the entire Lua standard library. So instead Lua uses its own pattern matching implementation in less than 500 lines of code (which means it has less power than a traditional POSIX implementation).
   7. Lua also starts indexing at 1 instead of 0. This is really not too hard to get used to but it is unique to Lua.